<template>
  <section class="space-y-6">
    ...
    <form
      v-if="showForm"
      class="card grid gap-3 p-5 md:grid-cols-2"
    ...
    <div class="space-y-3">
      ...
      >
        <div class="mb-3">
          <p
            class="text-sm font-semibold uppercase tracking-wider text-uva-orange"
          >
...
const calendarWeeks = computed(() => {
  ...
  const cells = Array.from({ length: totalCells }).map((_, index) => {
    ...
    const label = new Date(year, month, dayOfMonth).toLocaleDateString(
      undefined,
      {
        month: "short",
        day: "numeric",
